...
class AddReservationTabViewDataController extends GetxController {
  ...
  static Future<AddReservationTabViewDataController> create() async {
    final AddReservationTabViewDataController controller =
        AddReservationTabViewDataController._();
    await controller._fetchAvailableTables(date: DateTime.now(), timeSlotStart: 14, timeSlotEnd: 16);
    return controller;
  }
  ...
  final RxList<RestaurantTable> listenableAvailableTables = <RestaurantTable>[].obs;
  ...
  Future<List<RestaurantTable>> _fetchAvailableTables({
    ...
    try {
      ...
      try{
        ...
        _availableTables.value = dataMap['available_tables']!.map<RestaurantTable>(
            (tableMap) => RestaurantTable.fromMap(tableMap)
        ).toList();

        listenableAvailableTables.value = dataMap['available_tables']!.map<RestaurantTable>(
                (tableMap) => RestaurantTable.fromMap(tableMap)
        ).toList();
  ...
  Future<void> fetchAvailableTables({
    ...
    try{
      await _fetchAvailableTables(date: date, timeSlotStart: timeSlotStart, timeSlotEnd: timeSlotEnd);
    }
    on NetworkException catch(e){
      //TODO:handle errors
    }
